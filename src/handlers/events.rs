    ...
    -> Option<impl Stream<Item = Result<impl warp::sse::ServerSentEvent, warp::Error>> + Send + 'static>
{
    ...
    Some(ch_rx.map(|message| {
        Ok(warp::sse::data(format!("{},{}", message.0, message.1)))
    }))
}

pub async fn events_num(poll_id: db::PollID, pool: Pool, ctx: EventContext) -> Result<Box<dyn warp::Reply>, warp::Rejection> {
    if !try_500!(db::valid_poll_id_num(pool, &poll_id).await) {
